    ...
    <section className="dashboard__wrapper">
      ...
      <div className="games__wrapper">
        ...
        <div className="newgame__container">
          ...
          <p className="text__m-normal">Voeg al je leerlingen toe aan een spel en genereer een unieke deelbare link.</p>
          <div className="spacer__s"></div>
          <a className="button__primary">maak nieuw spel aan</a>
        </div>
          ...
          {games.map((game) => {
            ...
            return (
              <Window key={game.link} text={`${WEBSITE_URL}/game/${game.link}`}>
                <div>
                  <p className="bold"> {game.name} - {date.toLocaleDateString("be-nl")}</p>
                  <p>{game.students.length} Leerlingen</p>
                </div>

                <a
                  className="button__primary"
                  onClick={(e) => {setGame(game.id); setId(2);}}>
                  statistieken
                </a>
              </Window>
            );
